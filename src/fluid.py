...
class VEFluid:
    def __init__(self, particle_count : int, boundary_box: Tuple[float, float]):
        ...
        self.fluidHashGrid = HashGrid.FluidHashGrid(self.INTERACTION_RADIUS, self.particles)
    ...
    def neighbourSearch(self, mousePos):
        ...
        mouseVector2 = Data.Vector2(mousePos[0], mousePos[1])*(1/8) # i.e. 1/scale factor established on main
    ...
    def neighbourSearchB(self, mousePos):
        ...
        mouseVector2 = Data.Vector2(mousePos[0], mousePos[1])*0.2 # i.e. 1/scale factor established on main
       
        self.particles[0].position = mouseVector2
        cellsAroundMouse: List[Data.Particle] = self.fluidHashGrid.getNeighboursOfParticleIdx(0)

        self.resetParticleColours()

        if len(cellsAroundMouse) > 0:
            for p in cellsAroundMouse:
                distanceSquared = (p.position - mouseVector2).magnitudeSquared()
                p.colour = (255,200,0) if distanceSquared < self.INTERACTION_RADIUS_2 else p.colour
    ...
    def compute_next_velocity(self, dt):
        inverse_dt = 1.0/dt
        for p in self.particles:
            velocity: Data.Vector2 = (p.position - p.prevPosition) * (inverse_dt)
            p.velocity = velocity
    ...
    def doubleDensityRelaxation(self, dt):
        for i in range(len(self.particles)):
            density = 0
            densityNear = 0
            neighbours : List[Data.Particle] = self.fluidHashGrid.getNeighboursOfParticleIdx(i)
            ...
            for n in neighbours:
                ...
                if q < 1.0:
                    r_ij.normalize()
                    displacementTerm : float = math.pow(dt, 2)*(pressure * (1-q) + pressureNear*math.pow((1-q), 2))
        ...
        for i in range(len(self.particles)):
            ...
            for particle_B in neighbours:
                ...
                if q < 1.0:
                    r_ij.normalize()
                    one_minus_q = 1 - q
                    displacementTerm = (dt ** 2) * (pressure * one_minus_q + pressureNear * (one_minus_q ** 2))
                    displacementVector = r_ij * displacementTerm
                    particle_B.position += displacementVector * 0.5
                    particleADisplacement -= displacementVector * 0.5
